...
- 已知故障、未执行验证与风险：
  ...
  - 标签缓存为内存级。当时认为恢复后冷启动不影响正确性（首个检查真实执行）；**2026-09-09 修正**：独立审阅 F02 证明冷启动改变消费与训练，R1-2 已改为恢复时从持久记录重建标签缓存（见 REVIEW_FIXES_CORE_20260909.md）。
  - **2026-09-09 补充（R1 修复）**：未提交提案的检查抽签 RNG 状态此前不持久（崩溃窗重放会改变后续检查流，审阅 F01），现抽样后的 bit-generator 状态随 proposal 事件持久化并在恢复时复原；evaluation 提交此前不绑定数据库行（崩溃窗可产生被当作权威的孤立行，审阅 F03），现提交绑定 row_id/内容摘要且写入幂等；窗口内的缓存复用此前会被误判为消费丢失（审阅 F07），现消费集合按全日志构建。
