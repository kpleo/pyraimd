...
(empty) output directory, runs the configured NVE steps in this process
and exits.  The run is continued by resume_run.py — in a new process,
possibly after the whole directory has been moved.
"""
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, required=True,
                        help="new, empty directory that becomes the run")
    ...
    if (output / "run.toml").exists():
        raise SystemExit(f"{output} already contains a run; choose a new, "
                         "empty directory")
    ...
    config_path.write_text(
        CONFIG_TEMPLATE.format(model=json.dumps(str(model)),
                               steps=args.steps),
        encoding="utf-8")
